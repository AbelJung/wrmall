	...
	*/
	...
	public ModelAndView update(@ModelAttribute TB_PDINFOXM productInfo, TB_MBINFOXM tb_meinfoxm,BindingResult result, SessionStatus status, TB_ODINFOXM tb_odinfoxm, HttpServletRequest request, Model model) throws Exception {
		...
		*/ 
		...
		TB_COATFLXD commonFile = new TB_COATFLXD();												
		commonFile.setATFL_ID(rtnObj.getATFL_ID());
		commonFile.setATFL_SEQ(rtnObj.getRPIMG_SEQ());
		TB_COATFLXD modell = (TB_COATFLXD) commonService.selectFile(commonFile);
		String strSTFL_NAME = modell.getSTFL_NAME().trim();		
		String originalFileName = modell.getORFL_NAME().trim();
		String strSTFL_PATH = modell.getSTFL_PATH().trim();
		String strFILE_EXT = modell.getFILE_EXT().trim().toLowerCase();
		String savePath = (request.getSession().getServletContext().getRealPath("/")).replace("\\", "/") + "upload/" +strSTFL_PATH;
		System.out.println("savePath======"+savePath);
		System.out.println("STFL_NAME======"+strSTFL_NAME);
		System.out.println("STFL_PATH======"+strSTFL_PATH);
		System.out.println("originalFileName======"+originalFileName);
		System.out.println("strFILE_EXT======"+strFILE_EXT);
		*/
		...
		catagoryInfo.setSUPR_ID("C00002");
