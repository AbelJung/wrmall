...
<div class="sub-title sub-title-underline">
	<h2>
		상품목록 <small class="ml_5">| 엄선된 상품만을 제공합니다.</small>
	</h2>
	<ul>
		<li><a href='${contextPath }/goods' class="sct_bg">Home</a></li>
...
<div class="panel panel-default panel-search">
	<div class="panel-body pos_r">
		<h5 class="search-title eng mg_n fl_l"  style="height:15px;">
			<span class="blue"></span>
		</h5>
		<form method="GET" action="${contextPath }/product/retailList?RETAIL_GUBN=${obj.RETAIL_GUBN}">
		<!-- <input type="hidden" name="schTxt_bef" /> -->
		<span class="pos_a" style="right:20px; top:8px">
			<a href="?RETAIL_GUBN=${obj.RETAIL_GUBN}&amp;sortGubun=PD_NAME&amp;sortOdr=asc&pagerMaxPageItems=${obj.pagerMaxPageItems}" class="btn btn-order" >상품명순</a>
			<%-- <a href="?CAGO_ID=${obj.CAGO_ID}&amp;schTxt=${param.schTxt }&amp;schGbn=${param.schGbn }&amp;sortGubun=CAGO_SORT&amp;sortOdr=asc" class="btn btn-order" >상품분류순</a> --%>
			<a href="?RETAIL_GUBN=${obj.RETAIL_GUBN}&amp;sortGubun=PD_PRICE&amp;sortOdr=asc&pagerMaxPageItems=${obj.pagerMaxPageItems}" class="btn btn-order" >낮은가격순</a>
			<a href="?RETAIL_GUBN=${obj.RETAIL_GUBN}&amp;sortGubun=PD_PRICE&amp;sortOdr=desc&pagerMaxPageItems=${obj.pagerMaxPageItems}" class="btn btn-order" >높은가격순</a>
			...
			<input type="hidden" name="CAGO_ID" value="${obj.CAGO_ID}"/>
			<input type="hidden" name="RETAIL_GUBN" value="${obj.RETAIL_GUBN}"/>
		</span>
		</form>
	</div>
</div>
