...
<!-- <script src="http://www.cjfls.co.kr/resources/js/addr_check.js"></script> -->
...
$(function() {
    ...
    $("#btnSave").click(function() {
		...
		var orderAmt = $("#ORDER_AMT").val();	//주문총금액
		var dlvyAmt = $("#DLVY_AMT").val();		//배송비
		var dapYn = 'Y';//$("#DAP_YN").val();		//배송비결제여부
		...
		for(var i=0; i<pdc_gbn.length;i++){
			if(pdc_gbn[i].value !='PDDC_GUBN_05'){		//추석선물세트만이 아닐때
				if (!addr_Daejun($("#POST_NUM").val()))	//기존로직(대전지역만 배송할때)
		        {
					alert('대전의 유성구,서구 지역만 배송 가능합니다. \n\r양해부탁드리겠습니다.');
		           return;
		        }
			}
		}
...
function fn_dlvy_cal(){	
	...
	for(var i=0; i<pdc_gbn.length;i++){
		//추석선물세트만이 아닐때
		if(pdc_gbn[i].value !='PDDC_GUBN_05'){		
			if (!addr_Daejun($("#POST_NUM").val()))	//기존로직
	        {
	           return;
	        }
		// 추석선물세트만 있을때 배송비 로직 변경
		}else{		
...
<%-- <form method="post" name="fregisterform" id="fregisterform" action="${contextPath }/order/orderPay"> --%>
